...
public:
    ...
    static Expr fuse(Expr container, Expr outer_loop, Expr inner_loop)
    {
        ...
        if (ol->range.as<Range>()->min.as<IntImm>()->value != 0 || il->range.as<Range>()->min.as<IntImm>()->value != 0)
        {
...
public:
    ...
    static Expr bind(Expr container, Expr loop, Expr thread)
    {
        ...
        if (t->thread_type == Thread::THREAD_TYPE::TIDx || t->thread_type == Thread::THREAD_TYPE::TIDy || t->thread_type == Thread::THREAD_TYPE::TIDz)
...
class Merger : public IRMutator
{

public:
    static Expr make(Expr original_domain, int axis)
    {
